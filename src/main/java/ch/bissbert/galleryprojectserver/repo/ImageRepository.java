...
public interface ImageRepository extends JpaRepository<Image, Integer> {
    @Query("select new Image(i.id, i.height, i.width, i.bitsPerPixel, i.compressionType, i.name, i.mimeType) from Image i")
    public Page<Image> findAllWithoutImages(Pageable pageable);

    @Query("select new Image(i.id, i.previewImage) from Image i where i.id = :id")
